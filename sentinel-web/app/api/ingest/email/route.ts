...
import { generateSafeFileName, hashBuffer } from '@/lib/utils';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      originalName = (file as File).name || 'upload.eml';
      isMultipart  = true;
    ...
    const savedPath = isMultipart
      ? path.join(INBOUND_DIR, `${ts}-${safeName}`)
      : path.join(INBOUND_DIR, `${ts}.txt`);
    ...
    await appendAudit({
      ...
      user:         'System Ingest',
      action:       'Case Created',
      details:      'Created from email ingest',
